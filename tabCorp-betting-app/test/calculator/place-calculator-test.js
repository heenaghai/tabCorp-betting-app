...
var placeCalculator = require('../../lib/calculator/place-calculator');
...
describe("Place Win Bets", function() {
    it("should return winning bets out of all exacta bets", function(done) {
      var output = new placeCalculator().calculateWinningBets(betsArray, raceResult);
      expect(placeWinningBets).to.be.ok;
      expect(output).to.have.property('1');
      expect(output).to.deep.equal(placeWinningBets);
      done();
    });
});