...
var winCalculator = require('../../lib/calculator/win-calculator');
...
describe("Win Product Win Bets", function() {
    it("should return winning bets out of all exacta bets", function(done) {
      var output = new winCalculator().calculateWinningBets(betsArray, raceResult);
      expect(winWinningBets).to.be.ok;
      expect(output).to.have.property('2');
      expect(output).to.deep.equal(winWinningBets);
      done();
    });
});